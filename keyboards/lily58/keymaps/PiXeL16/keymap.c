...
// Bongo Cat Animation - WPM-responsive animation
...
void animation_phase(void) {
    ...
    if (get_current_wpm() > IDLE_SPEED && get_current_wpm() < TAP_SPEED) {
        oled_write_raw_P(prep[0], ANIM_SIZE);  // prep animation
    }
...
bool oled_task_user(void) {
  if (is_keyboard_master()) {
    // Left OLED - Bongo Cat Animation with layout: mode, cat, WPM, letters
    
    // First line: Display current layer/mode
    oled_set_cursor(0, 0);
    oled_write_ln(read_layer_state(), false);
    
    // Render bongo cat animation (takes most of the screen)
    render_bongo_cat();
    
    // Bottom overlay: WPM and some info
    oled_set_cursor(0, 3);  // Bottom line
    char wpm_str[32];
